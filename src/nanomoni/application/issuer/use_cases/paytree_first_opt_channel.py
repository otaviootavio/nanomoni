"""Use cases for the issuer PayTree First Opt flow."""
...
from ....domain.issuer.entities import Account, PaytreeFirstOptPaymentChannel
from ....domain.issuer.repositories import AccountRepository, PaymentChannelRepository


class PaytreeFirstOptChannelService:
    """Service to manage opening and settling PayTree First Opt channels."""
    ...
    async def open_channel(
        ...
        if (
            dto.paytree_first_opt_root_b64 is None
            or dto.paytree_first_opt_unit_value is None
            or dto.paytree_first_opt_max_i is None
        ):
            raise ValueError(
                "PayTree First Opt fields are required for channel opening"
            )
        ...
        try:
            _ = b64_to_bytes(dto.paytree_first_opt_root_b64)
        except Exception as e:
            raise ValueError(f"Invalid paytree_first_opt_root_b64: {e}") from e

        max_owed = compute_cumulative_owed_amount(
            i=dto.paytree_first_opt_max_i,
            unit_value=dto.paytree_first_opt_unit_value,
        )
        if max_owed > dto.amount:
            raise ValueError(
                "PayTree First Opt max owed exceeds channel amount "
        ...
        channel = PaytreeFirstOptPaymentChannel(
            ...
            amount=dto.amount,
            balance=0,
            paytree_first_opt_root_b64=dto.paytree_first_opt_root_b64,
            paytree_first_opt_unit_value=dto.paytree_first_opt_unit_value,
            paytree_first_opt_max_i=dto.paytree_first_opt_max_i,
        )
        created = await self.channel_repo.create(channel)
        if not isinstance(created, PaytreeFirstOptPaymentChannel):
            raise RuntimeError(
                "Unexpected: persisted channel is not PayTree First Opt-enabled"
            )
        ...
        return PaytreeFirstOptOpenChannelResponseDTO(
            ...
            paytree_first_opt_root_b64=created.paytree_first_opt_root_b64,
            paytree_first_opt_unit_value=created.paytree_first_opt_unit_value,
            paytree_first_opt_max_i=created.paytree_first_opt_max_i,
        )

    async def settle_channel(
        self, channel_id: str, dto: PaytreeFirstOptSettlementRequestDTO
    ) -> CloseChannelResponseDTO:
        ...
        if not isinstance(channel, PaytreeFirstOptPaymentChannel):
            raise ValueError("Payment channel is not PayTree First Opt-enabled")
        ...
        if dto.i > channel.paytree_first_opt_max_i:
            raise ValueError("i exceeds PayTree First Opt max_i for this channel")
        ...
        try:
            ...
            raise ValueError(
                "Invalid vendor signature for PayTree First Opt settlement"
            )

        try:
            _ = b64_to_bytes(channel.paytree_first_opt_root_b64)
        except Exception as e:
            raise ValueError(f"Invalid PayTree First Opt root encoding: {e}") from e

        if not verify_paytree_proof(
            i=dto.i,
            leaf_b64=dto.leaf_b64,
            siblings_b64=dto.siblings_b64,
            root_b64=channel.paytree_first_opt_root_b64,
        ):
            raise ValueError("Invalid PayTree First Opt proof (root mismatch)")

        cumulative_owed_amount = compute_cumulative_owed_amount(
            i=dto.i, unit_value=channel.paytree_first_opt_unit_value
        )
    ...
    async def get_channel(
        ...
        if not isinstance(channel, PaytreeFirstOptPaymentChannel):
            raise ValueError("Payment channel is not PayTree First Opt-enabled")
        return PaytreeFirstOptPaymentChannelResponseDTO(**channel.model_dump())
