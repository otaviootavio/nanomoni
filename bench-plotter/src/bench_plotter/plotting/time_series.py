...
def calculate_optimal_window_size(timestamps: List[float], min_window_seconds: int = 1, multiplier: float = 2.0) -> float:
...
def create_windowed_plot(
    ...
    window_centers, window_averages = calculate_windowed_averages(timestamps, values, window_seconds)
    ...
    ax.plot(elapsed_centers, window_averages, 'b-', linewidth=2, label=f'{formatted_window}s Window Average')
    ...
    max_value = max(sample_values + window_averages) if sample_values or window_averages else 0
...
def create_windowed_plot_multi(
    ...
    for idx, series in enumerate(series_list):
        ...
        ax.plot(elapsed, values, color=color, linewidth=2, linestyle=linestyle, label=label)
