...
def get_dashboard_panels(panels_spec: List[Dict[str, Any]] | None = None) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        return get_dashboard_panels()
    except Exception:
        try:
            from dashboard_queries import get_dashboard_panels
            return get_dashboard_panels()
        except ImportError:
            print("Error: dashboard_queries.py not found. Please create it from your dashboard configuration.")
            return []


def determine_interval_type(test_intervals: List[Dict[str, Any]]) -> tuple[bool, Dict[str, List[Dict[str, Any]]]]:
    """
    ...
    if not is_single_interval:
        ...
        if len(modes) == 1:
            ...
            print(f"Multiple intervals detected ({len(test_intervals)}) with same mode, using statistical analysis")
            is_single_interval = False
        else:
            # Different modes - treat each interval individually
            print(f"Multiple intervals detected ({len(test_intervals)}) with different modes: {list(modes.keys())}")
    ...
    for interval_idx, interval in enumerate(test_intervals):
        ...
        try:
            result = query_with_fallbacks(
                ...
                instant_query_func=lambda query: asyncio.run(query_range(query=query, start_unix=start_time, end_unix=end_time))
            )
            
            payload_result = result.get('data', {}).get('result', []) if isinstance(result, dict) else []
...
def process_histogram_data_for_intervals(
    ...
    for interval_idx, interval in enumerate(test_intervals):
        ...
        try:
            ...
            result = asyncio.run(instant_query(
                query=expr
            ))
            ...
            if not vector_result:
                ...
                for item in matrix_result:
                    ...
                    if values:
                        last_value = values[-1]
                        if last_value and len(last_value) >= 2 and last_value[1] != 'NaN':
...
def is_tps_panel(panel_title: str, legend_format: str, expr: str) -> bool:
    ...
    if 'frequency distribution' in panel_title_lower or 'distribution' in panel_title_lower:
        return False
    
    return (
        panel_title_lower.find('tps') != -1 or
        legend_format_lower.find('tps') != -1 or
        expr_lower.find('tps') != -1 or
        panel_title_lower.find('duration') != -1 or
        panel_title_lower.find('quantile') != -1 or
        panel_title_lower.find('payment') != -1
    )
...
def create_tps_plot(
    ...
    if any_nonzero:
        series_list = [s for s in temp_series if any((v is not None and float(v) != 0) for v in s.get('values', []))]
    else:
...
def create_single_series_plots(
    ...
    for i, series_data in enumerate(runs_data):
        ...
        if output_path is None:
            output_path = str(section_dir / f"{safe_panel_title}_{safe_legend}{mode_suffix}.png")
        else:
            # If output_path is provided, add mode suffix
            output_path = str(Path(output_path).parent / f"{Path(output_path).stem}{mode_suffix}.png")
...
def process_dashboard(
    ...
    for panel in non_tps_panels:
        ...
        for target in panel.get('targets', []):
            ...
            if runs_data and not is_histogram_query(expr, panel_title, legend_format):
                ...
                if is_tps_panel(panel_title, legend_format, expr):
                    output_path = str(section_dir / f"{safe_panel_title}{filename_suffix}.png")
                    create_tps_plot(
                        runs_data, panel_title, legend_format, 
                        section_dir, safe_panel_title, safe_legend, output_path, plot_title
                    )
                    continue
                
                if is_histogram_query(expr, panel_title, legend_format):
                    print(f"Detected histogram query, processing as bar chart (overlaid modes)")
                    
                    all_histogram_data = process_histogram_data_for_intervals(expr, test_intervals)
                    
                    if all_histogram_data and len(all_histogram_data) == len(test_intervals):
                        output_path = str(section_dir / f"{safe_panel_title}{filename_suffix}_overlay.png")
                        create_overlaid_histogram_plot(
                    ...
                    if len(test_intervals) > 1:
                        ...
                        create_single_series_plots(
                            runs_data, test_intervals, panel_title, legend_format,
                            section_dir, safe_panel_title, safe_legend, window_seconds, output_path, plot_title
                        )
                else:
                    output_path = str(section_dir / f"{safe_panel_title}{filename_suffix}.png")
                    # Extract unit from title for y-axis label
    ...
    for panel_title, panels in tps_panels_by_title.items():
        if len(panels) == 1:
            ...
            print(f"Grouping TPS panel '{panel_title}' with {len(panels)} payment modes")
            ...
            if not has_quantiles:
                print(f"  No quantiles found, creating single combined plot")
                combined_runs_data = []
                ...
                if combined_runs_data:
                    ...
                    if any_nonzero:
                        series_list = [s for s in combined_series if any((v is not None and float(v) != 0) for v in s.get('values', []))]
                    else:
                ...
                for quantile, targets in targets_by_quantile.items():
                    ...
                    if combined_runs_data:
                        ...
                        output_path = str(section_dir / f"{safe_panel_title}_{safe_quantile}.png")
    ...
    for panel_title, panels in distribution_panels_by_title.items():
        if len(panels) == 1:
            ...
            # Multiple panels with same title - combine payment modes
            print(f"Grouping distribution panel '{panel_title}' with {len(panels)} payment modes")
            ...
            for panel in panels:
                for target in panel.get('targets', []):
                    ...
                    print(f"  Fetching histogram data for: {panel_title} ({mode_label})")
                    ...
                    all_histogram_data = process_histogram_data_for_intervals(expr, matching_intervals)
