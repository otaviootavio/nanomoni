...
def process_histogram_data(runs_data: List[Dict[str, Any]]) -> tuple[List[str], List[float]]:
    """
...
def is_histogram_query(expr: str, title: str, legend_format: str) -> bool:
    ...
    if 'quantile' in title_lower or 'p99' in title_lower or 'p95' in title_lower or 'p50' in title_lower:
        return False
...
def create_histogram_plot(
    ...
    # Create bar plot
    bars = ax.bar(range(len(buckets)), counts, color=colors, edgecolor='black', linewidth=0.5)
...
def create_overlaid_histogram_plot(
    ...
    for idx, mode in enumerate(modes):
        ...
        ax.plot(x_vals, y_vals, color=color, linewidth=2, linestyle=linestyle, label=mode)
