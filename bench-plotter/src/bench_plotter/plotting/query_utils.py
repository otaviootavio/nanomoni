...
def query_with_fallbacks(
    ...
    """
    def run_query(qs: str):
        try:
            return asyncio.run(query_range_func(query=qs, start_unix=start_time, end_unix=end_time))
    ...
    q_no_container_label = re.sub(r",?\s*container_label_com_docker_compose_service\s*=\s*\"[^\"]*\"\s*,?", "", q_sanitized)
    ...
    try:
        ...
        instant_res = instant_payload.get('data', {}).get('result', []) if isinstance(instant_payload, dict) else []
        if instant_res:
