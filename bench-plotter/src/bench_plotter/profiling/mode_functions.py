...
"endpoint" (macro, whole-request) span vs. the "crypto"/"db read"/"db write"
(micro) spans for each payment mode.
...
imports no ``json``, so no serialization frame can ever be a descendant of a
primitive, and the buckets are disjoint. The repository's own Python (key
f-strings, result dicts, reference parsing, and all dump/parse marshalling --
``model_dump_json``, ``model_validate_json``, ``loads``, ``dumps``) lands in
``other``; an earlier version of this taxonomy broke marshalling out into its
own "serialize" bucket, but the per-mode name lists needed to do that
consistently drifted apart (e.g. some modes' lists omitted ``model_validate``/
``loads``), silently undercounting some modes' bucket relative to others, so
the category was removed rather than reconciled.
...
Each bucket's list uses leaf-level names only (no ancestor/descendant pairs in
the same list) so ``sum_ticks_within`` does not double-count.
"""
