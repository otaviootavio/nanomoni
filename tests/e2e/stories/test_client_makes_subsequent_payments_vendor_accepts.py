...
async def test_client_makes_subsequent_payments_vendor_accepts(
    ...
    Each payment must have a higher owed_amount than the previous one.
    """
    ...
    await vendor_client.receive_payment(computed_id, first_payment)

    # When: Client sends subsequent payments with increasing amounts
    subsequent_amounts = [200, 350, 500, 750]
    last_owed = 100

    for owed_amount in subsequent_amounts:
        ...
        assert owed_amount > last_owed, "Owed amount must be strictly increasing"
        last_owed = owed_amount
...
async def test_client_makes_sequence_of_payments_vendor_accepts_all(
    ...
    for owed_amount in payment_sequence:
        payment_envelope = client.create_payment_envelope(
            computed_id, vendor_public_key_der_b64, owed_amount
        )
        payment_response = await vendor_client.receive_payment(
