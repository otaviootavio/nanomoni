...
async def test_client_makes_decreasing_payment_vendor_rejects(
    ...
    """
    # Given: A channel with a payment (owed_amount=200)
    client = ClientActor()
    ...
    first_payment = client.create_payment_envelope(
        computed_id, vendor_public_key_der_b64, 200
    )
    await vendor_client.receive_payment(computed_id, first_payment)

    # When: Client tries to send a payment with lower owed_amount
    decreasing_payment = client.create_payment_envelope(
        computed_id, vendor_public_key_der_b64, 150
    )
